...
export async function didIssuesTokens() {
    ...
    let { document } = await createDid(client, secretManager, storage);
    let authorityDid = document.id();

    console.log("Created DID in", Date.now() - start, "ms");
    start = Date.now();
    ...
    console.log("Got rent structure in", Date.now() - start, "ms");
    start = Date.now();
    ...
    console.log("Updated DID output in", Date.now() - start, "ms");
    start = Date.now();
    ...
    console.log("Built alias output in", Date.now() - start, "ms");
    start = Date.now();
    ...
    const tokenId: string = Utils.computeTokenId(
        authorityDid.toAliasId(),
        1,
        tokenScheme.getType()
    );
    console.log("Computed token ID in", Date.now() - start, "ms");
    start = Date.now();
    // Create a token foundry that represents carbon credits.
    var carbonCreditsFoundry: FoundryOutput = await client.buildFoundryOutput({
        ...
        unlockConditions: [
            new ImmutableAliasAddressUnlockCondition(new AliasAddress(authorityDid.toAliasId())),
        ],
    });
    console.log("Built foundry output in", Date.now() - start, "ms");
    start = Date.now();
    ...
    console.log("Computed storage deposit in", Date.now() - start, "ms");
    start = Date.now();
    // Publish the foundry.
    const [blockId, block] = await client.buildAndPostBlock(secretManager, {
        outputs: [authorityAliasOutput, carbonCreditsFoundry],
    });
    console.log("Built and posted block in", Date.now() - start, "ms");
    start = Date.now();

    await client.retryUntilIncluded(blockId);
    console.log("Foundry published in", Date.now() - start, "ms");
    start = Date.now();
    // ===================================
    ...
    const aliasUnlockCondition: ImmutableAliasAddressUnlockCondition = carbonCreditsFoundry
        .getUnlockConditions()
        .find(
            (unlockCondition) =>
    ...
    const authorityAliasId: string = (
        aliasUnlockCondition.getAddress() as AliasAddress
    ).getAliasId();
    ...
    console.log("Resolved authority DID in", Date.now() - start, "ms");
    start = Date.now();
    // =========================================================
    ...
    // const companyAddressBech32: string = (
    //     await new SecretManager(secretManager).generateEd25519Addresses({
    //         accountIndex: 0,
    //         range: {
    //             start: 1,
    //             end: 2,
    ...
    const companyAddressBech32 = "rms1qqp9sp9cz03zdjq7py7amk8275sxjf2wx5prkukehak8f8397lem5cel5m6";
    const companyAddress = Utils.parseBech32Address(companyAddressBech32);
    console.log("Company address bech32:", companyAddressBech32);
    console.log("Company address:", companyAddress);
    console.log("Company address created in", Date.now() - start, "ms");
    start = Date.now();
    ...
    const basicOutput: BasicOutput = await client.buildBasicOutput({
        ...
        unlockConditions: [
            ...
            new ExpirationUnlockCondition(new AliasAddress(authorityAliasId), tomorrow),
        ],
    });
    ...
    console.log("Sent carbon credits in", Date.now() - start, "ms");
    start = Date.now();
    await client.retryUntilIncluded(blockId2);
    console.log("publish credits in", Date.now() - start, "ms");
    start = Date.now();
